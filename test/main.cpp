...
typedef unsigned int int32;//存储正数位数
typedef int int31;//存储正数位数

void load(char* dataByteP) {
	//num_byte_p = dataByteP / 8;
	for (int32 num = 0; num < MAX_NUMBER; num++) {
...
bool exist(int32 inputNum, char* dataByteP) {
...
int main() {
	...
	int32 max_bit = MAX_NUMBER;//(int 4字节*8=32位)2的32次方>40亿,因位图算法把每一个数按1个字节1个数占存储,0-0,n-n位
	int31 max_byte = max_bit / BITS_PER_BYTE + 1;//(int 4字节*8=32位,符号占1位)2的31次方>(40亿/8+1=5亿+1):7/8+1=1
	...
	printf("请输入要测试的数字:");
	int32 inputNum = -1;
	scanf_s("%ud", &inputNum, sizeof(int32));//unsigned int和int存储的位数一样多(32)
	//输出
